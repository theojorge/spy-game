...
export default function App() {
  ...
  useEffect(() => {
    const loadThemes = async () => {
      try {
        ...
        const savedSelectedThemes =
          await AsyncStorage.getItem("selectedThemes");
        ...
        if (savedThemes && savedSelectedThemes) {
          ...
          console.log(
            "No saved themes or selected themes, initializing with defaults",
          );
          ...
          await AsyncStorage.setItem(
            "selectedThemes",
            JSON.stringify(Object.keys(themes)),
          );
        }
      } catch (error) {
        console.error(
          "Error loading themes or selectedThemes from AsyncStorage",
          error,
        );
      }
    };
  ...
  const saveNewTheme = () => {
    if (!newTheme || wordsList.length === 0) {
      Alert.alert(
        "Error",
        "Por favor ingresa una temática y al menos una palabra",
      );
      return;
    }
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View style={styles.checkboxContainer}>
                    <Checkbox
                      status={
                        selectedThemes.includes(item) ? "checked" : "unchecked"
                      }
                  ...
                  <View style={styles.wordContainer}>
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        ...
                        const updatedWords = wordsList.filter(
                          (_, i) => i !== index,
                        );
